# -*- coding: utf-8 -*-
"""
NEUROCITY DEVELOPER EDITION 3.0
...
async function doWork(id){try{let x=await api('/api/work',{job_id:id});toast(x.message);loadJobs();loadHome()}catch(e){toast(e.message)}}
...
async function createBusiness(){try{await api('/api/business/create',{name:$('bn').value,category:$('bc').value,description:$('bd').value,capital:+$('bcap').value,job_title:$('bjob').value,job_reward:+$('br').value});closeModal();toast('Бизнес создан 🚀');show('business');loadBusinesses();loadHome()}catch(e){toast(e.message)}}
...
async function upgradeBusiness(id){try{let x=await api('/api/business/upgrade',{id});toast(x.message);closeModal();loadBusinesses();loadHome()}catch(e){toast(e.message)}}
...
async function loadProfile(){
 ...
 <div class="grid2"><button class="action" onclick="openProfileEdit()">✏️<small>Редактор</small></button><button class="action" onclick="openBank()">🏦<small>Банк</small></button><button class="action" onclick="openFriends()">👥<small>Друзья</small></button><button class="action" onclick="openInventory()">🎒<small>Инвентарь</small></button></div>
...
async function saveProfile(){try{await api('/api/profile/update',{name:$('pn').value,bio:$('pb').value,avatar:$('pa').value,cover:$('pc').value});closeModal();toast('Профиль обновлён');loadProfile();loadHome()}catch(e){toast(e.message)}}
...
async function boot(){
 ...
 loadCars(); loadBusinesses(); loadJobs(); loadMarket(); loadSeason();
}
boot();
</script>
</body>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/index.html"):
            ...
            self.send_json({"ok": True, "app": "NeuroCity", "version": "5.0 Production"})
        else:
            self.send_error(404)
...
def main():
    init_pool()
    init_db()
    print("NEUROCITY DEVELOPER EDITION 5.0 PRODUCTION")
