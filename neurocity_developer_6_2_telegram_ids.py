...
class PGConnection:
    ...
    def close(self):
        global DB_POOL, DB_SEM
        if self.conn is not None and not self._released:
            conn = self.conn
            self._released = True
            self.conn = None
            try:
...
def init_pool():
    ...
    max_conn = max(5, int(os.getenv("DB_POOL_MAX", "25")))
    DB_POOL = pool.ThreadedConnectionPool(2, max_conn, DATABASE_URL, connect_timeout=15)
    DB_SEM = threading.BoundedSemaphore(max_conn)
    print(f"[DB] PostgreSQL pool ready: max={max_conn}")
...
def db():
    ...
    if DB_POOL is None:
        init_pool()
    # Wait briefly instead of failing immediately with "connection pool exhausted".
    acquired = DB_SEM.acquire(timeout=float(os.getenv("DB_POOL_WAIT", "8")))
    if not acquired:
        raise RuntimeError("База занята. Попробуй ещё раз через секунду.")
    try:
...
def init_db():
    with LOCK:
        ...
        -- Telegram user IDs are 64-bit. Upgrade legacy INTEGER user-reference
        -- columns from older NeuroCity versions so IDs above 2,147,483,647 work.
        ALTER TABLE IF EXISTS businesses ALTER COLUMN owner_id TYPE BIGINT;
        ALTER TABLE IF EXISTS work ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS products ALTER COLUMN seller_id TYPE BIGINT;
        ALTER TABLE IF EXISTS purchases ALTER COLUMN buyer_id TYPE BIGINT;
        ALTER TABLE IF EXISTS purchases ALTER COLUMN seller_id TYPE BIGINT;
        ALTER TABLE IF EXISTS inventory ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS cars ALTER COLUMN seller_id TYPE BIGINT;
        ALTER TABLE IF EXISTS car_purchases ALTER COLUMN buyer_id TYPE BIGINT;
        ALTER TABLE IF EXISTS car_purchases ALTER COLUMN seller_id TYPE BIGINT;
        ALTER TABLE IF EXISTS friends ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS friends ALTER COLUMN friend_id TYPE BIGINT;
        ALTER TABLE IF EXISTS chat ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS feed ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS business_reviews ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS follows ALTER COLUMN follower_id TYPE BIGINT;
        ALTER TABLE IF EXISTS follows ALTER COLUMN target_id TYPE BIGINT;
        ALTER TABLE IF EXISTS achievements ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS profile_items ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS notifications ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS user_quests ALTER COLUMN user_id TYPE BIGINT;
        ALTER TABLE IF EXISTS user_clothing ALTER COLUMN user_id TYPE BIGINT;
        """)
...
async function boot(){
 await init();
 // Do not fire many database requests at once on startup.
 // Other sections load when the player opens them.
}
boot();
</script>
</body>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/index.html"):
            ...
            self.send_json({"ok": True, "app": "NeuroCity", "version": "6.1 Stable"})
        else:
            self.send_error(404)
...
def main():
    init_pool()
    init_db()
    print("NEUROCITY DEVELOPER EDITION 6.1 • STABLE")
