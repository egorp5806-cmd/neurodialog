...
B=[
('СТАРТ','start',0,''),
('Казань','property',600,'green'),
('Шанс','chance',0,''),
('Самара','property',700,'green'),
('Налог города','tax',500,''),
('Санкт-Петербург','station',1200,''),
('Уфа','property',900,'blue'),
('Шанс','chance',0,''),
('Нижний Новгород','property',1000,'blue'),
('Екатеринбург','property',1100,'blue'),
('Парк отдыха','free',0,''),
('Сочи','property',1300,'pink'),
('Электростанция','utility',1500,''),
('Минск','property',1400,'pink'),
('Алматы','property',1500,'pink'),
('Москва','station',1200,''),
('Краснодар','property',1700,'orange'),
('Шанс','chance',0,''),
('Ростов-на-Дону','property',1800,'orange'),
('Владивосток','property',1900,'orange'),
('Свободная площадь','free',0,''),
('Пермь','property',2100,'red'),
('Шанс','chance',0,''),
('Новосибирск','property',2200,'red'),
('Омск','property',2300,'red'),
('Томск','station',1200,''),
('Красноярск','property',2500,'yellow'),
('Иркутск','property',2600,'yellow'),
('Водоканал','utility',1500,''),
('Якутск','property',2700,'yellow'),
('Штрафная зона','jail',0,''),
('Токио','property',2900,'purple'),
('Сингапур','property',3000,'purple'),
('Шанс','chance',0,''),
('Дубай','property',3200,'purple'),
('Лондон','station',1200,''),
('Событие','chance',0,''),
('Париж','property',3500,'black'),
('Налог на роскошь','tax',1000,''),
('Нью-Йорк','property',4000,'black'),
('Лос-Анджелес','property',4200,'black'),
('Шанс','chance',0,''),
('Сеул','property',4400,'black'),
('Сидней','property',4600,'black'),
('Аэропорт','station',1600,''),
('Шанхай','property',4800,'black'),
('Стамбул','property',5000,'black')
]
...
BOARD_SIZE=len(B)
MAX_ROLLS=60
V13_SEASON='MEGA CITY • S1'
V13_TASKS=[('roll10','🎲 Бросить кубик 10 раз',10,600),('buy2','🏠 Купить 2 города',2,900),('build2','🏗️ Улучшить 2 здания',2,1200),('chat3','💬 Отправить 3 сообщения',3,400)]
V13_ITEMS={'cars':['🚗 Городской седан','🏎️ Спорткар','🚙 Премиум SUV','🚕 Такси магната'],'clothes':['👕 Street','🧥 Business','🥋 Neon','👑 Tycoon'],'house':['🛋️ Дизайнерский интерьер','🖼️ Галерея','🌴 Пентхаус','🏰 Особняк']}
...
def initdb():
 ...
 try:
  with c.cursor() as x:
   ...
   # V13 persistent meta progression
   x.execute('''CREATE TABLE IF NOT EXISTS player_meta(user_id BIGINT PRIMARY KEY REFERENCES players(user_id) ON DELETE CASCADE,level INT DEFAULT 1,xp BIGINT DEFAULT 0,coins BIGINT DEFAULT 0,season_xp BIGINT DEFAULT 0,streak INT DEFAULT 0,last_daily DATE,avatar TEXT DEFAULT '🧑‍💼',clothes TEXT DEFAULT '👕 Street',car TEXT DEFAULT '🚗 Городской седан',house_level INT DEFAULT 1,businesses JSONB DEFAULT '[]',inventory JSONB DEFAULT '[]',achievements JSONB DEFAULT '[]',map TEXT DEFAULT 'Russia',updated_at TIMESTAMPTZ DEFAULT now())''')
   x.execute('''CREATE TABLE IF NOT EXISTS player_tasks(user_id BIGINT REFERENCES players(user_id) ON DELETE CASCADE,task_id TEXT,progress INT DEFAULT 0,claimed BOOL DEFAULT false,season TEXT,updated_at TIMESTAMPTZ DEFAULT now(),PRIMARY KEY(user_id,task_id,season))''')
   x.execute('''CREATE TABLE IF NOT EXISTS season_stats(user_id BIGINT REFERENCES players(user_id) ON DELETE CASCADE,season TEXT,score BIGINT DEFAULT 0,wins INT DEFAULT 0,PRIMARY KEY(user_id,season))''')
   x.execute('CREATE INDEX IF NOT EXISTS idx_season_score ON season_stats(season,score DESC)')
  c.commit()
 finally: put(c)
...
def code():
 while 1:
  ...
  try:
   with c.cursor() as x:
    x.execute('SELECT 1 FROM rooms WHERE code=%s',(s,))
...
def snapshot(code,uid):
 ...
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   ...
   board=[{'id':i,'name':n,'type':t,'price':p,'group':g,'city':t in ('property','station','utility')} for i,(n,t,p,g) in enumerate(B)]
...
def start():
 ...
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   ...
   st=dict(r['state'] or {});st['rolls']=0;st['max_rolls']=MAX_ROLLS;st['winner']=None;st['winner_stats']=[];st['log']=(st.get('log') or [])+['🎲 Игра началась!'];x.execute("UPDATE rooms SET status='playing',state=%s WHERE code=%s",(Json(st),cd))
  c.commit();return jsonify(snapshot(cd,u))
 finally:put(c)
...
def roll():
 ...
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   ...
   d1,d2=random.randint(1,6),random.randint(1,6);steps=d1+d2;old=int(a['pos']);new=(old+steps)%BOARD_SIZE;money=int(a['money'])+(2000 if new<old else 0)
   path=[(old+i)%BOARD_SIZE for i in range(1,steps+1)]
   x.execute('UPDATE room_players SET pos=%s,money=%s,xp=xp+%s WHERE room_code=%s AND user_id=%s',(new,money,50+steps*5,cd,u))
   p=B[new];st=dict(r['state'] or {});log=list(st.get('log') or []);rolls=int(st.get('rolls',0))+1;st['rolls']=rolls;st['max_rolls']=int(st.get('max_rolls') or MAX_ROLLS);log.append(f"{a['name']} выбросил {d1}+{d2} и попал на «{p[0]}».")
   ...
   x.execute("SELECT COUNT(*) n FROM room_players WHERE room_code=%s AND bankrupt=false",(cd,));alive=int(x.fetchone()['n']);winner=None;winner_stats=[]
   finish_reason=None
   if alive<=1:
    x.execute("SELECT user_id,name,money,props,houses FROM room_players WHERE room_code=%s AND bankrupt=false LIMIT 1",(cd,));wr=x.fetchone();winner=wr['name'] if wr else None
    if wr:
     props=list(wr['props'] or []);houses=dict(wr['houses'] or {})
     wealth=int(wr['money'])+sum(int(B[int(q)][2]) for q in props if 0<=int(q)<BOARD_SIZE)+sum(max(500,int(B[int(q)][2]*.35))*int(v) for q,v in houses.items() if str(q).isdigit() and 0<=int(q)<BOARD_SIZE)
     winner_stats=[{'name':wr['name'],'wealth':wealth,'properties':len(props)}]
     x.execute("UPDATE room_players SET wins=wins+1,xp=xp+1000 WHERE room_code=%s AND user_id=%s",(cd,wr['user_id']))
    finish_reason='last_player';log.append(f"🏆 Победитель: {winner}" if winner else "🏁 Игра завершена.");status='finished';nxt=idx
   elif rolls>=int(st['max_rolls']):
    # Time limit: winner is the player with the greatest total wealth.
    for q in ps:
     if q['bankrupt']: continue
     props=[int(v) for v in (q['props'] or []) if str(v).isdigit()]
     houses=dict(q['houses'] or {})
     wealth=int(q['money'])+sum(int(B[v][2]) for v in props if 0<=v<BOARD_SIZE)+sum(max(500,int(B[int(pid)][2]*.35))*int(level) for pid,level in houses.items() if str(pid).isdigit() and 0<=int(pid)<BOARD_SIZE)
     winner_stats.append({'user_id':int(q['user_id']),'name':q['name'],'wealth':wealth,'properties':len(props)})
    winner_stats.sort(key=lambda z:(z['wealth'],z['properties']),reverse=True)
    if winner_stats:
     winner=winner_stats[0]['name'];finish_reason='max_rolls'
     x.execute("UPDATE room_players SET wins=wins+1,xp=xp+1000 WHERE room_code=%s AND user_id=%s",(cd,winner_stats[0]['user_id']))
    winner_stats=winner_stats[:3]
    log.append(f"🏆 Победитель по капиталу: {winner}" if winner else "🏁 Игра завершена.");status='finished';nxt=idx
   else:
   ...
   if winner_stats:st['winner_stats']=winner_stats
   if finish_reason:st['finish_reason']=finish_reason
   x.execute('UPDATE rooms SET turn=%s,status=%s,state=%s,updated_at=now() WHERE code=%s',(nxt,status,Json(st),cd))
  c.commit();v13_progress(u,'roll10',1);return jsonify(snapshot(cd,u))
 finally:put(c)
@app.post('/api/rooms/buy')
def buy():
 u,n=auth();body=request.json or {};cd=str(body.get('code','')).upper();pid=int(body.get('property_id',-1));p=B[pid] if 0<=pid<BOARD_SIZE else None
 ...
 try:
  ...
  c.commit();v13_progress(u,'buy2',1);return jsonify(snapshot(cd,u))
 finally:put(c)
...
def build():
 ...
 try:
  ...
  c.commit();v13_progress(u,'build2',1);return jsonify(snapshot(cd,u))
 finally:put(c)
...
def chat():
 ...
 try:
  ...
  c.commit();v13_progress(u,'chat3',1);return jsonify(snapshot(cd,u))
 finally:put(c)
...
def auction_create():
 ...
 try:
  with c.cursor() as x:
   ...
   if not r:return jsonify(error='Комната не найдена'),404
   if cell<0 or cell>=BOARD_SIZE or B[cell][1] not in ('property','station','utility'):return jsonify(error='Эту клетку нельзя выставить'),400
...
def ensure_meta(uid,name=None):
 c=conn()
 try:
  with c.cursor() as x:
   if name: x.execute('INSERT INTO players(user_id,name) VALUES(%s,%s) ON CONFLICT(user_id) DO UPDATE SET name=EXCLUDED.name,last_seen=now()',(uid,name))
   x.execute("INSERT INTO player_meta(user_id) VALUES(%s) ON CONFLICT(user_id) DO NOTHING",(uid,))
   for tid,_,_,_ in V13_TASKS:
    x.execute("INSERT INTO player_tasks(user_id,task_id,season) VALUES(%s,%s,%s) ON CONFLICT DO NOTHING",(uid,tid,V13_SEASON))
   x.execute("INSERT INTO season_stats(user_id,season) VALUES(%s,%s) ON CONFLICT DO NOTHING",(uid,V13_SEASON))
  c.commit()
 finally: put(c)
...
def meta_row(uid):
 ...
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   x.execute('SELECT * FROM player_meta WHERE user_id=%s',(uid,)); m=x.fetchone()
   x.execute('SELECT task_id,progress,claimed FROM player_tasks WHERE user_id=%s AND season=%s',(uid,V13_SEASON)); ts=x.fetchall()
   x.execute('SELECT score,wins FROM season_stats WHERE user_id=%s AND season=%s',(uid,V13_SEASON)); ss=x.fetchone()
   x.execute('SELECT COUNT(*) n FROM room_players WHERE user_id=%s AND wins>0',(uid,)); wins=int(x.fetchone()['n'])
   tasks=[]
   for tid,title,target,reward in V13_TASKS:
    q=next((z for z in ts if z['task_id']==tid),None); tasks.append({'id':tid,'title':title,'target':target,'reward':reward,'progress':int(q['progress']) if q else 0,'claimed':bool(q['claimed']) if q else False})
   return {'meta':dict(m),'tasks':tasks,'season':dict(ss or {'score':0,'wins':wins})|{'name':V13_SEASON},'inventory':{'cars':V13_ITEMS['cars'],'clothes':V13_ITEMS['clothes'],'house':V13_ITEMS['house']}}
 finally: put(c)

@app.get('/api/v13/dashboard')
def v13_dashboard():
 u,n=auth()
 if not u:return jsonify(error='Telegram authorization required'),401
 ensure_meta(u,n)
 return jsonify(**meta_row(u))

@app.post('/api/v13/profile')
def v13_profile():
 u,n=auth(); b=request.json or {}
 if not u:return jsonify(error='Telegram authorization required'),401
 allowed=set(['🧑‍💼','🧑‍🚀','🧑‍🎨','🧑‍🔬','🧑‍💻','🧑‍🍳','🦊','🐯','🐼','🤖','👽','🦸'])
 avatar=str(b.get('avatar','🧑‍💼'))[:8]; clothes=str(b.get('clothes','👕 Street'))[:40]; car=str(b.get('car','🚗 Городской седан'))[:40]; mp=str(b.get('map','Russia'))[:20]
 if avatar not in allowed:return jsonify(error='Недопустимый персонаж'),400
 if clothes not in V13_ITEMS['clothes'] or car not in V13_ITEMS['cars'] or mp not in ('Russia','Europe','Asia','USA','World'):return jsonify(error='Недопустимый предмет или карта'),400
 ensure_meta(u,n); c=conn()
 try:
  with c.cursor() as x:x.execute('UPDATE player_meta SET avatar=%s,clothes=%s,car=%s,map=%s,updated_at=now() WHERE user_id=%s',(avatar,clothes,car,mp,u))
  c.commit(); return jsonify(**meta_row(u))
 finally:put(c)

@app.post('/api/v13/daily')
def v13_daily():
 u,n=auth()
 if not u:return jsonify(error='Telegram authorization required'),401
 ensure_meta(u,n); c=conn()
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   x.execute('SELECT last_daily,streak FROM player_meta WHERE user_id=%s FOR UPDATE',(u,));m=x.fetchone(); today=time.strftime('%Y-%m-%d'); last=str(m['last_daily']) if m['last_daily'] else ''
   if last==today:return jsonify(**meta_row(u),claimed_today=True)
   streak=int(m['streak'] or 0)+1 if last else 1; reward=min(3000,200*streak);x.execute('UPDATE player_meta SET coins=coins+%s,streak=%s,last_daily=%s,season_xp=season_xp+100,updated_at=now() WHERE user_id=%s',(reward,streak,today,u))
  c.commit(); return jsonify(**meta_row(u),daily_reward=reward,claimed_today=True)
 finally:put(c)

@app.post('/api/v13/task/<task_id>/claim')
def v13_claim(task_id):
 u,n=auth()
 if not u:return jsonify(error='Telegram authorization required'),401
 ensure_meta(u,n); spec=next((q for q in V13_TASKS if q[0]==task_id),None)
 if not spec:return jsonify(error='Задание не найдено'),404
 c=conn()
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   x.execute('SELECT * FROM player_tasks WHERE user_id=%s AND task_id=%s AND season=%s FOR UPDATE',(u,task_id,V13_SEASON));q=x.fetchone()
   if not q or q['claimed'] or int(q['progress'])<spec[2]:return jsonify(error='Задание ещё не выполнено'),400
   x.execute('UPDATE player_tasks SET claimed=true WHERE user_id=%s AND task_id=%s AND season=%s',(u,task_id,V13_SEASON));x.execute('UPDATE player_meta SET coins=coins+%s,season_xp=season_xp+%s,updated_at=now() WHERE user_id=%s',(spec[3],spec[3]//2,u));x.execute('UPDATE season_stats SET score=score+%s WHERE user_id=%s AND season=%s',(spec[3]//2,u,V13_SEASON))
  c.commit();return jsonify(**meta_row(u))
 finally:put(c)

@app.get('/api/v13/leaderboard')
def v13_global_leaderboard():
 u,n=auth()
 if not u:return jsonify(error='Telegram authorization required'),401
 c=conn()
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   x.execute("SELECT p.user_id,p.name,m.avatar,m.level,m.coins,m.season_xp,s.score,s.wins FROM players p JOIN player_meta m ON m.user_id=p.user_id LEFT JOIN season_stats s ON s.user_id=p.user_id AND s.season=%s ORDER BY COALESCE(s.score,0) DESC,m.coins DESC LIMIT 50",(V13_SEASON,));return jsonify(players=x.fetchall(),season=V13_SEASON)
 finally:put(c)

@app.post('/api/v13/business/buy')
def v13_business_buy():
 u,n=auth(); b=request.json or {}; kind=str(b.get('kind',''))
 catalog={'shop':('🏪 Магазин',2500,120),'restaurant':('🍔 Ресторан',6000,320),'hotel':('🏨 Отель',12000,700),'bank':('🏦 Банк',25000,1500),'factory':('🏭 Завод',40000,2600)}
 if kind not in catalog:return jsonify(error='Бизнес не найден'),404
 ensure_meta(u,n); c=conn()
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   x.execute('SELECT * FROM player_meta WHERE user_id=%s FOR UPDATE',(u,));m=x.fetchone(); businesses=list(m['businesses'] or []); price=catalog[kind][1]
   if any(z.get('kind')==kind for z in businesses):return jsonify(error='Этот бизнес уже куплен'),400
   if int(m['coins'])<price:return jsonify(error=f'Нужно {price} 🪙'),400
   businesses.append({'kind':kind,'name':catalog[kind][0],'level':1,'income':catalog[kind][2]});x.execute('UPDATE player_meta SET coins=coins-%s,businesses=%s,updated_at=now() WHERE user_id=%s',(price,Json(businesses),u))
  c.commit();return jsonify(**meta_row(u))
 finally:put(c)

@app.post('/api/v13/business/upgrade')
def v13_business_upgrade():
 u,n=auth();b=request.json or {};kind=str(b.get('kind',''));ensure_meta(u,n);c=conn()
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   x.execute('SELECT * FROM player_meta WHERE user_id=%s FOR UPDATE',(u,));m=x.fetchone();bs=list(m['businesses'] or []);z=next((q for q in bs if q.get('kind')==kind),None)
   if not z:return jsonify(error='Сначала купите бизнес'),400
   if z['level']>=10:return jsonify(error='Максимальный уровень'),400
   cost=1000*z['level']
   if int(m['coins'])<cost:return jsonify(error=f'Нужно {cost} 🪙'),400
   z['level']+=1;z['income']*=2;x.execute('UPDATE player_meta SET coins=coins-%s,businesses=%s WHERE user_id=%s',(cost,Json(bs),u))
  c.commit();return jsonify(**meta_row(u))
 finally:put(c)

@app.post('/api/v13/collect')
def v13_collect():
 u,n=auth();ensure_meta(u,n);c=conn()
 try:
  with c.cursor(cursor_factory=RealDictCursor) as x:
   x.execute('SELECT businesses FROM player_meta WHERE user_id=%s FOR UPDATE',(u,));m=x.fetchone();income=sum(int(z.get('income',0))*int(z.get('level',1)) for z in (m['businesses'] or []));
   if income<=0:return jsonify(error='Нет бизнеса для сбора'),400
   x.execute('UPDATE player_meta SET coins=coins+%s,season_xp=season_xp+50 WHERE user_id=%s',(income,u))
  c.commit();return jsonify(**meta_row(u),collected=income)
 finally:put(c)

def v13_progress(uid,task_id,amount=1):
 c=conn()
 try:
  with c.cursor() as x:
   x.execute("INSERT INTO player_tasks(user_id,task_id,season,progress) VALUES(%s,%s,%s,%s) ON CONFLICT(user_id,task_id,season) DO UPDATE SET progress=LEAST(player_tasks.progress+EXCLUDED.progress,50),updated_at=now()",(uid,task_id,V13_SEASON,amount))
   x.execute("UPDATE player_meta SET season_xp=season_xp+%s,xp=xp+%s,level=GREATEST(1,1+((xp+%s)/1000)::int) WHERE user_id=%s",(amount*10,amount*10,amount*10,uid))
  c.commit()
 finally: put(c)
...
<section id="v13" class="screen"><div class="title">🌍 City Tycoon V13</div><div class="sub">Мир игрока: профиль, сезоны, задания, бизнес и глобальный рейтинг.</div><div class="card"><div class="row"><div><div class="sub">Уровень</div><div id="vLevel" class="money">1 ⭐</div></div><div><div class="sub">V13 Coins</div><div id="vCoins" class="money">0 🪙</div></div></div><div id="vSeason" class="sub" style="margin-top:8px"></div><button class="primary" onclick="dailyV13()">🎁 Забрать ежедневный бонус</button></div><div class="card"><b>👤 Персонаж и стиль</b><div class="row"><select id="vAvatar"><option>🧑‍💼</option><option>🧑‍🚀</option><option>🧑‍🎨</option><option>🧑‍🔬</option><option>🧑‍💻</option><option>🦊</option><option>🐯</option><option>🐼</option><option>🤖</option><option>🦸</option></select><select id="vClothes"><option>👕 Street</option><option>🧥 Business</option><option>🥋 Neon</option><option>👑 Tycoon</option></select><select id="vCar"><option>🚗 Городской седан</option><option>🏎️ Спорткар</option><option>🚙 Премиум SUV</option><option>🚕 Такси магната</option></select></div><button class="primary" onclick="saveV13Style()">💾 Сохранить стиль</button></div><div class="card"><b>🎯 Задания сезона</b><div id="vTasks" class="log"></div></div><div class="card"><b>🏢 Мои бизнесы</b><div id="vBusinesses" class="log"></div><button class="secondary" onclick="collectV13()">💰 Собрать доход</button><div class="sub" style="margin-top:10px">Купить: <button class="secondary" onclick="buyBiz('shop')">🏪</button><button class="secondary" onclick="buyBiz('restaurant')">🍔</button><button class="secondary" onclick="buyBiz('hotel')">🏨</button><button class="secondary" onclick="buyBiz('bank')">🏦</button><button class="secondary" onclick="buyBiz('factory')">🏭</button></div></div><div class="card"><b>🌎 Карты</b><div class="sub">Россия • Европа • Азия • США • Мир</div><div class="row"><button class="secondary" onclick="saveV13Map('Russia')">🇷🇺</button><button class="secondary" onclick="saveV13Map('Europe')">🇪🇺</button><button class="secondary" onclick="saveV13Map('Asia')">🌏</button><button class="secondary" onclick="saveV13Map('USA')">🇺🇸</button><button class="secondary" onclick="saveV13Map('World')">🌐</button></div></div><div class="card"><b>🏆 Глобальный рейтинг</b><div id="vLeaderboard" class="log"></div></div></section>
<nav class="nav"><button onclick="show('home')"><i>🏠</i><b>Главная</b></button><button onclick="roomCode?show(g?.status==='playing'||g?.status==='finished'?'game':'room'):show('join')"><i>🎲</i><b>Игра</b></button><button onclick="show('profile')"><i>👤</i><b>Профиль</b></button><button onclick="show('v13');loadV13()"><i>🚀</i><b>Мир</b></button></nav><div id="toast" class="toast"></div></div>
<script>
...
function cityIcon(name){let m={'Казань':'🕌','Самара':'🌉','Санкт-Петербург':'🏛️','Уфа':'🌆','Нижний Новгород':'🏰','Екатеринбург':'🏙️','Сочи':'🌴','Минск':'🏛️','Алматы':'⛰️','Москва':'🏰','Краснодар':'🌴','Ростов-на-Дону':'🌉','Владивосток':'🌊','Пермь':'🏙️','Новосибирск':'🏙️','Омск':'🏙️','Томск':'🏛️','Красноярск':'🏔️','Иркутск':'🏔️','Якутск':'❄️','Токио':'🗼','Сингапур':'🌇','Дубай':'🏙️','Лондон':'🎡','Париж':'🗼','Нью-Йорк':'🗽','Лос-Анджелес':'🌴','Сеул':'🏙️','Сидней':'🌉','Шанхай':'🏙️','Стамбул':'🕌'};return m[name]||'🏙️'}
function cityArt(name,group){let grad={'green':['#0f766e','#34d399'],'blue':['#1d4ed8','#60a5fa'],'pink':['#9d174d','#f472b6'],'orange':['#9a3412','#fb923c'],'red':['#991b1b','#f87171'],'yellow':['#854d0e','#facc15'],'purple':['#6b21a8','#c084fc'],'black':['#111827','#64748b']}[group]||['#1e293b','#64748b'];let icon=cityIcon(name);let svg=`<svg xmlns="http://www.w3.org/2000/svg" width="240" height="90" viewBox="0 0 240 90"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${grad[0]}"/><stop offset="1" stop-color="${grad[1]}"/></linearGradient></defs><rect width="240" height="90" rx="12" fill="url(#g)"/><circle cx="205" cy="20" r="11" fill="#fff" opacity=".35"/><path d="M0 72 L25 55 42 68 62 42 82 69 105 48 126 68 149 37 169 68 191 50 214 68 240 44 V90 H0Z" fill="#050811" opacity=".55"/><text x="12" y="30" font-size="23">${icon}</text><text x="12" y="80" font-family="Arial" font-size="15" font-weight="700" fill="white">${name}</text></svg>`;return 'data:image/svg+xml;charset=UTF-8,'+encodeURIComponent(svg)}
function renderGame(){let p=g.players.find(x=>x.user_id==g.me),a=g.players[g.turn];if(!p)return;$('money').textContent=Number(p.money).toLocaleString('ru-RU')+' 🪙';$('turn').textContent=a?esc(a.name):'—';let rounds=Number(g.state?.rolls||0),max=Number(g.state?.max_rolls||60);$('roundInfo').textContent=`Ход ${rounds} / ${max}`;let c=g.board[p.pos];$('pos').textContent=c.name;$('info').textContent=c.price?`Цена ${c.price.toLocaleString('ru-RU')} 🪙 • ${c.city?'Городская недвижимость':''}`:'';let ownerP=g.players.find(q=>(q.props||[]).map(Number).includes(c.id));let mep=g.players.find(q=>q.user_id==g.me);let lv=ownerP?Number((ownerP.houses||{})[String(c.id)]||0):0;let rentTxt=c.type==='property'?`Аренда уровня ${lv}: ${[60,120,360,900,1600,2500][Math.min(lv,5)]} 🪙`:(c.type==='station'?'Станция: аренда зависит от числа станций':'');$('propertyDetail').innerHTML=(c.city?`<img class="cityimg" style="width:180px;height:70px;margin:7px 0" src="${cityArt(c.name,c.group)}">`:'')+(ownerP?`Владелец: ${ownerP.avatar||'🧑‍💼'} ${esc(ownerP.name)} • ${rentTxt}`:rentTxt);let last=g.state?.last_roll;$('board').innerHTML=g.board.map(c=>{let op=g.players.find(q=>(q.props||[]).map(Number).includes(c.id));let hs=op?Number((op.houses||{})[String(c.id)]||0):0;let toks=g.players.filter(q=>q.pos===c.id&&!q.bankrupt).map(q=>`<span class="tok" title="${esc(q.name)}">${esc(q.avatar||'🧑‍💼')}</span>`).join('');let special=!c.city;let pic=c.city?`<img class="cityimg" src="${cityArt(c.name,c.group)}" alt="">`:'';return `<div class="cell ${c.city?'citycell':''} ${special?'special':''} ${c.id===p.pos?'me':''} ${last?.to===c.id?'active float':''}">${c.group?`<div class="bar" style="background:${g.colors[c.group]}"></div>`:''}${pic}<b>${esc(c.name)}</b><small>${c.price?c.price.toLocaleString('ru-RU')+' 🪙':''}</small>${hs?`<div class="level">${hs>=5?'🏨': '🏠'.repeat(Math.min(hs,4))} LVL ${hs}</div>`:''}${toks}</div>`}).join('');$('log').innerHTML=(g.state?.log||[]).slice().reverse().map(x=>`<div>${esc(x)}</div>`).join('');renderChat();renderTrades();renderDice();$('rollBtn').disabled=a?.user_id!==g.me||g.status!=='playing';let ws=g.state?.winner_stats||[];$('winnerBox').innerHTML=g.status==='finished'?`<div class="winner">🏆 Победитель: ${esc(g.state?.winner||'не определён')}<div class="sub" style="margin-top:6px">${g.state?.finish_reason==='max_rolls'?'Победа по капиталу после лимита ходов':'Последний оставшийся игрок'}</div>${ws.length?`<div class="winnerstats">${ws.map((w,i)=>`<div class="winnerrow"><span>${i===0?'👑':(i+1)+'.'} ${esc(w.name)}</span><b>${Number(w.wealth).toLocaleString('ru-RU')} 🪙</b></div>`).join('')}</div>`:''}${g.host_id==g.me?`<button class="primary" style="margin-top:10px" onclick="rematch()">🔁 Реванш</button>`:''}</div>`:'';renderAchievements(p);loadLeaderboard()}
...
async function buyHere(){try{let p=g.players.find(x=>x.user_id==g.me),c=g.board[p.pos];g=await api('/api/rooms/buy',{method:'POST',body:JSON.stringify({code:roomCode,property_id:c.id})});renderGame();toast('🏠 Участок куплен')}catch(e){toast(e.message)}}async function buildHere(){try{let p=g.players.find(x=>x.user_id==g.me),c=g.board[p.pos];g=await api('/api/rooms/build',{method:'POST',body:JSON.stringify({code:roomCode,property_id:c.id})});renderGame();toast('🏗️ Здание улучшено')}catch(e){toast(e.message)}}async function loadV13(){try{let d=await api('/api/v13/dashboard');$('vLevel').textContent=(d.meta.level||1)+' ⭐';$('vCoins').textContent=Number(d.meta.coins||0).toLocaleString('ru-RU')+' 🪙';$('vSeason').textContent='🏆 '+d.season.name+' • '+Number(d.season.score||0)+' XP';$('vTasks').innerHTML=d.tasks.map(t=>`<div class="card"><b>${esc(t.title)}</b><div class="sub">${t.progress}/${t.target} • награда ${t.reward} 🪙</div>${t.progress>=t.target&&!t.claimed?`<button class="primary" onclick="claimV13('${t.id}')">Забрать</button>`:''}${t.claimed?'<span class="pill">✅ Получено</span>':''}</div>`).join('');$('vBusinesses').innerHTML=(d.meta.businesses||[]).map(b=>`<div class="pl"><span style="font-size:25px">${esc(b.name.split(' ')[0])}</span><div style="flex:1"><b>${esc(b.name)}</b><div class="sub">Уровень ${b.level} • доход ${Number(b.income).toLocaleString('ru-RU')} 🪙</div></div><button class="secondary" onclick="upgradeV13('${b.kind}')">⬆️</button></div>`).join('')||'<div class="sub">Пока нет бизнеса.</div>';let lb=await api('/api/v13/leaderboard');$('vLeaderboard').innerHTML=lb.players.map((p,i)=>`<div class="pl"><b>${i+1}. ${esc(p.avatar||'🧑‍💼')} ${esc(p.name)}</b><span style="margin-left:auto">${Number(p.score||0).toLocaleString('ru-RU')} XP</span></div>`).join('')||'<div class="sub">Пока нет игроков.</div>'}catch(e){toast(e.message)}}async function saveV13Style(){try{await api('/api/v13/profile',{method:'POST',body:JSON.stringify({avatar:$('vAvatar').value,clothes:$('vClothes').value,car:$('vCar').value})});loadV13();toast('👤 Стиль сохранён')}catch(e){toast(e.message)}}async function dailyV13(){try{let d=await api('/api/v13/daily',{method:'POST',body:'{}'});loadV13();toast(d.daily_reward?`🎁 +${d.daily_reward} 🪙`: 'Бонус уже получен')}catch(e){toast(e.message)}}async function claimV13(id){try{await api('/api/v13/task/'+id+'/claim',{method:'POST',body:'{}'});loadV13();toast('🎁 Награда получена')}catch(e){toast(e.message)}}async function upgradeV13(k){try{await api('/api/v13/business/upgrade',{method:'POST',body:JSON.stringify({kind:k})});loadV13();toast('🏢 Бизнес улучшен')}catch(e){toast(e.message)}}async function buyBiz(k){try{await api('/api/v13/business/buy',{method:'POST',body:JSON.stringify({kind:k})});loadV13();toast('🏢 Бизнес куплен')}catch(e){toast(e.message)}}async function collectV13(){try{let d=await api('/api/v13/collect',{method:'POST',body:'{}'});loadV13();toast(`💰 +${d.collected} 🪙`)}catch(e){toast(e.message)}}async function saveV13Map(map){try{await api('/api/v13/profile',{method:'POST',body:JSON.stringify({map})});loadV13();toast('🌍 Карта изменена')}catch(e){toast(e.message)}}
init();loadRooms();setInterval(()=>{if(!$('home').classList.contains('on'))return;loadRooms()},4000);
</script></body></html>'''
