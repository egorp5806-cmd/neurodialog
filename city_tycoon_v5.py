...
async function api(u,o={}){
  const target = new URL(u, window.location.href).toString();
  const headers=Object.assign({},o.headers||{});
  if(!(o.body instanceof FormData)) headers['Content-Type']='application/json';
  if(tg?.initData){
    const init=String(tg.initData).replace(/[\r\n]/g,'');
    if(init) headers['X-Telegram-Init-Data']=init;
  }
  o={...o,headers};
  let r;
  try{ r=await fetch(target,o); }
  catch(err){ throw new Error('Не удалось отправить запрос. Проверь интернет и открой Mini App заново.'); }
  let d;
  try{ d=await r.json(); }catch(_){ d={error:'Сервер вернул некорректный ответ'}; }
  if(!r.ok){let e=Error(d.error||'Ошибка сервера');Object.assign(e,d);throw e}
  return d
}
