# -*- coding: utf-8 -*-
import os,json,time,random,hmac,hashlib,urllib.parse,urllib.request,threading,traceback
...
def api_or_html_error(e):
 print('[UNHANDLED ERROR]',repr(e)); traceback.print_exc()
...
def create():
 ...
 try:
  with c.cursor() as x:
   x.execute('INSERT INTO rooms(code,host_id,status,turn,state,updated_at) VALUES(%s,%s,%s,%s,%s,now())',
             (cd,u,'waiting',0,Json({'log':[f'{n} создал комнату']})))
   x.execute('INSERT INTO room_players(room_code,user_id,slot,name,money,pos,props,houses,bankrupt,avatar,xp,wins,jail_turns,jailed,mortgages,badges) VALUES(%s,%s,%s,%s,15000,0,%s,%s,false,%s,0,0,0,false,%s,%s)',
             (cd,u,0,n,Json([]),Json({}),'🧑‍💼',Json({}),Json([])))
  c.commit()
 except Exception as e:
  try:c.rollback()
  except Exception:pass
  print('[CREATE ROOM ERROR]',repr(e)); traceback.print_exc()
  return jsonify(error='Не удалось создать комнату',details=str(e)),500
 finally:
  put(c)
 try:
  s=snapshot(cd,u)
  if not s:return jsonify(error='Комната создана, но не удалось загрузить её данные'),500
  return jsonify(s)
 except Exception as e:
  print('[CREATE ROOM SNAPSHOT ERROR]',repr(e)); traceback.print_exc()
  return jsonify(error='Комната создана, но сервер не смог вернуть её данные',details=str(e)),500
...
async function api(u,o={}){o.headers=Object.assign({'Content-Type':'application/json'},o.headers||{});if(tg?.initData)o.headers['X-Telegram-Init-Data']=tg.initData;let r=await fetch(u,o),raw=await r.text(),d=null;try{d=raw?JSON.parse(raw):null}catch(_){d=null}if(!r.ok){let msg=d?.error||(`Сервер вернул HTTP ${r.status}`);if(d?.details)msg+=': '+d.details;let e=Error(msg);Object.assign(e,d||{});throw e}if(!d){throw Error('Сервер вернул некорректный ответ')}return d}
...
async function bidAuction(id){
 let v=prompt('Ваша ставка 🪙');
 if(!v)return;
 try{
  await api('/api/rooms/auction/bid',{method:'POST',body:JSON.stringify({code:roomCode,auction_id:id,bid:Number(v)})});
...
async function finishAuction(id){
 try{
  g=await api('/api/rooms/auction/finish',{method:'POST',body:JSON.stringify({code:roomCode,auction_id:id})});
  renderGame();
 }catch(e){toast(e.message)}
}
